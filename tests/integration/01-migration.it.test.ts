...
import { LEGACY_SHOW_TITLE, legacyShow, readPreMigrationSnapshot } from "./harness";
...
describe("Migration auf Prod-Daten", () => {
  ...
  it("hängt alle bisherigen Fotoerlaubnisse unverändert an „Die unendliche Geschichte“", async () => {
    ...
    expect(await prisma.photoConsent.count({ where: { showId: "show2027" } })).toBe(0);
  });
  ...
  it("gibt allen aktiven Mitgliedern eine aktive Mitgliedschaft", async () => {
    ...
    expect(await prisma.productionMembership.count({ where: { showId: "show2027" } })).toBe(0);
  });
  ...
  it("ordnet jedes Onboarding einer Produktion zu, keins „???“", async () => {
    expect(await prisma.productionOnboarding.count({ where: { showId: "show2027" } })).toBe(0);
