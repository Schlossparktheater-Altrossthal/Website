...
describe("aggregateHttpMetrics", () => {
  ...
  it("computes summary statistics and peak hours", () => {
    const requests = [
      createRequest({ offsetMinutes: 5, durationMs: 120, payloadBytes: 1_024, route: "/" }),
      createRequest({ offsetMinutes: 15, durationMs: 180, payloadBytes: 2_048, route: "/angebot" }),
      createRequest({ offsetMinutes: 25, durationMs: 90, payloadBytes: 128, route: "/robots.txt" }),
      createRequest({
        offsetMinutes: 45,
        durationMs: 100,
        payloadBytes: 64,
    ...
    expect(result.summary.successfulRequests).toBe(6);
    expect(result.summary.clientErrorRequests).toBe(1);
    expect(result.summary.serverErrorRequests).toBe(1);
    expect(result.summary.averageDurationMs).toBeCloseTo(182.5, 5);
    expect(result.summary.averagePayloadBytes).toBeCloseTo(1_368, 5);
    ...
    expect(result.summary.frontendRequests).toBe(4);
    expect(result.summary.membersRequests).toBe(2);
    expect(result.summary.apiRequests).toBe(2);
    expect(result.summary.frontendAvgResponseMs).toBeCloseTo(122.5, 5);
    ...
    expect(result.summary.guestRequests).toBe(2);
    expect(result.summary.guestAvgResponseMs).toBeCloseTo(150, 5);
    expect(result.summary.botRequests).toBe(2);
    expect(result.summary.botAvgResponseMs).toBeCloseTo(95, 5);
    expect(result.summary.botBlockedRequests).toBe(1);
