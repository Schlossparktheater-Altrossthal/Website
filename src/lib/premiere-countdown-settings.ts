...
export type PremiereCountdownSettingsRecord = Awaited<
  ReturnType<typeof readPremiereCountdownSettings>
>;
...
function parseTermineJson(value: Prisma.JsonValue): PremiereCountdownSettingsTermin[] {
  ...
  return value.filter(
    (item): item is PremiereCountdownSettingsTermin => !!item && typeof item === "object",
  ) as PremiereCountdownSettingsTermin[];
}

function buildFourTermine(
  value: Prisma.JsonValue,
  countdownTarget: Date | null,
): PremiereCountdownSettingsTermin[] {
  const parsed = parseTermineJson(value);
  const base = Array.from({ length: 4 }).map((_, index) => ({
    datum: "",
    uhrzeit: "",
    label: `Vorstellung ${index + 1}`,
  }));
  parsed.slice(0, 4).forEach((item, index) => {
    base[index] = {
      datum: item.datum ?? "",
      uhrzeit: item.uhrzeit ?? "",
      label: item.label ?? `Vorstellung ${index + 1}`,
    };
  });
