...
function deriveArea(pathname: string): AnalyticsRequestArea {
  ...
  if (pathname.startsWith("/mitglieder") || pathname.startsWith("/members")) return "members";
  return "public";
}
...
export function trackRequest(request: Request) {
  let url: URL;
  try {
  ...
  if (!shouldTrackPath(pathname, method)) return;

  const route = normalizeRoute(pathname);
  const area = deriveArea(route);
  const payloadBytes = parseContentLength(request);

  requestMeta.set(request, { startedAt: Date.now(), route, area, payloadBytes, method });
}

export function finalizeRequest(request: Request, response: Response) {
