  ...
  (
    ...
    const toggleSidebar = React.useCallback(() => {
      const shouldUseMobile =
        isMobile ||
        (typeof window !== "undefined" && window.matchMedia(SIDEBAR_MOBILE_BREAKPOINT).matches);
...
>(
  (
    ...
    const mobileContentStyle = React.useMemo(() => {
      ...
      return styleProp ? ({ ...styleProp, ...widthStyle } as React.CSSProperties) : widthStyle;
    }, [computedWidthValue, styleProp]);
    ...
    const handleTouchEnd = () => {
      ...
      const isClosingSwipe = (side === "left" && deltaX < 0) || (side === "right" && deltaX > 0);
    ...
    return (
      ...
      >
        <div
          className={cn(
            "relative w-[var(--sidebar-width)] min-h-[calc(100dvh-var(--header-height))] bg-transparent transition-[width] duration-200 ease-linear",
        ...
        <div
          className={cn(
            "fixed bottom-0 top-[var(--header-height)] z-10 hidden h-[calc(100dvh-var(--header-height))] w-[var(--sidebar-width)] transition-[left,right,width] duration-200 ease-linear lg:flex",
            side === "left"
...
>(({ className, ...props }, ref) => {
  return (
    <Input
      ...
      data-sidebar="input"
      className={cn(
        "h-9 w-full bg-background text-sm shadow-none focus-visible:ring-2 focus-visible:ring-sidebar-ring",
        className,
      )}
      {...props}
    />
...
>(({ className, asChild = false, ...props }, ref) => {
  ...
  return (
    <Comp
      ...
      className={cn(
        "flex h-8 shrink-0 items-center rounded-md px-2 text-[11px] font-semibold uppercase tracking-wide text-sidebar-foreground/70 outline-none ring-sidebar-ring transition-[margin,opacity] duration-200 ease-linear focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0",
