...
const headingVariants = cva("text-balance antialiased", {
  variants: {
    level: {
...
const Heading = React.forwardRef<HTMLHeadingElement, HeadingProps>(
  ({ className, level = "h2", tone, align, weight, asChild = false, ...props }, ref) => {
...
const Text = React.forwardRef<HTMLElement, TextProps>(
  (
    { className, variant = "body", tone, align, weight, uppercase, asChild = false, ...props },
    ref,
  ) => {
    const resolvedVariant = variant ?? "body";
    const Component = (
      asChild ? Slot : (textDefaultElement[resolvedVariant] ?? "p")
    ...
    return (
      <Component
        ...
        className={cn(
          textVariants({ variant: resolvedVariant, tone, align, weight, uppercase }),
          className,
        )}
        {...props}
      />
