...
export function RealtimeStatus({ rehearsalId, showPresence = true }: RealtimeStatusProps) {
  ...
  const handleAttendanceEvent = useCallback((event: AttendanceUpdateMessage) => {
    ...
    toast.info(`${actorLabel} hat die Anwesenheit${targetLabel} auf „${statusLabel}“ gesetzt`, {
  ...
  useAttendanceRealtime(rehearsalId || null, handleAttendanceEvent);
  ...
  const getConnectionBadge = () => {
    switch (connectionStatus) {
      ...
      case "connecting":
        return (
          <Badge variant="secondary" className="border-warning/45 bg-warning/15 text-warning">
            Verbinde...
          </Badge>
        );
      case "error":
          ...
          <div className="border-t pt-3">
            ...
            <div className="space-y-1">
              {presentUsers.length === 0 ? (
                <p className="text-xs text-muted-foreground">Keine anderen Benutzer online</p>
              ) : (
  ...
  if (recentUpdates.length === 0) {
    return (
      <Card>
        ...
        <CardContent>
          <p className="text-sm text-muted-foreground">Keine aktuellen Updates</p>
        </CardContent>
      </Card>
    );
  }
        ...
        {recentUpdates.map((update) => (
          <div key={update.id} className="flex items-start gap-2 rounded-lg bg-muted/40 p-2">
            ...
            <div className="flex-1 min-w-0 space-y-1">
              <p className="text-sm">
                <span className="font-medium">{formatUserId(update.actorUserId)}</span> hat die
                Anwesenheit
                {update.targetUserId ? (
                  <>
                    {" "}
