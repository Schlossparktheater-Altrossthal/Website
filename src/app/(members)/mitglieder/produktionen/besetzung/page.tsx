...
export default async function ProduktionsBesetzungPage() {
  ...
  try {
    ...
    const castingCount = characters.reduce(
      (acc, character) => acc + (character.castings?.length ?? 0),
      0,
    );
    const headerStats = [
        ...
        <section className="grid gap-6 xl:grid-cols-2">
          {characters.length === 0 ? (
            ...
            characters.map((character) => {
              const castings = character.castings ?? [];
              const sortedCastings = [...castings].sort((a, b) => {
