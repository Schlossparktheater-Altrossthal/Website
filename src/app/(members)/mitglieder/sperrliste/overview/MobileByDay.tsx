...
function selectDayBuckets(people: OverviewPerson[], dayCols: DayColumn[], holidays: HolidayIndicator[]): DayBucket[] {
  return dayCols.map((column, index) => {
    const entries: DayEntry[] = people.map((person) => ({ person, cell: person.days[index] }));
...
export function MobileByDay({ people, dayCols, holidays }: MobileByDayProps) {
  const buckets = useMemo(() => selectDayBuckets(people, dayCols, holidays), [people, dayCols, holidays]);
  return (
    <div className="space-y-2">
      {buckets.map((bucket) => (
        <div key={bucket.column.key} className="border rounded-lg p-2">
