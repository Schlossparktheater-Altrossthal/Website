            ...
            <header className="sticky top-0 z-10 bg-card backdrop-blur-md border-b border-border/40 shadow-sm">
              <div className="flex items-center justify-between px-3 py-2">
                ...
                <small className="text-[11px] text-muted-foreground">
                  {bucket.available.length} können · {bucket.limited.length} eingeschränkt ·{" "}
                  {bucket.blocked.length} gesperrt
                </small>
              </div>
            </header>
                ...
                <>
                  ...
                  {bucket.available.length > 0 && (
                    <div className="space-y-2">
                      {/* Schauspieler */}
                      {(() => {
                        const actorsAvailable = bucket.available.filter(
                          ({ person }) => person.group === "actors",
                        );
                      ...
                      {/* Beides */}
                      {(() => {
                        const bothAvailable = bucket.available.filter(
                          ({ person }) => person.group === "both",
                        );
                      ...
                      {/* Gewerke */}
                      {(() => {
                        const crewAvailable = bucket.available.filter(
                          ({ person }) => person.group === "crew",
                        );
                        ...
                        return (
                          <div key="crew-available">
                            <div className="flex items-center gap-2 px-2 py-1">
                              ...
                              <span className="text-[10px] font-semibold uppercase tracking-wider text-success-foreground">
                                Gewerke
                              </span>
                            </div>
                  ...
                  {bucket.limited.length > 0 && (
                    <div className="space-y-2">
                      {[
                        ...
                        const filtered = bucket.limited.filter(
                        ...
                        return (
                          <div key={group}>
                            <div className="flex items-center gap-2 px-2 py-1">
                              <div
                                className={`h-0.5 w-1 rounded-full bg-gradient-to-b ${
                                  color === "blue"
                  ...
                  {bucket.blocked.length > 0 && (
                    <div className="space-y-2">
                      {[
                        ...
                        const filtered = bucket.blocked.filter(
                          ({ person }) => person.group === group,
                        );
                        if (filtered.length === 0) return null;
                        return (
                  ...
                  {bucket.available.length > 0 && (
                    <ul className="divide-y divide-border/30 rounded-lg border border-success/40 bg-[color:var(--spl-ok-bg)]">
                      {bucket.available.map(({ person, cell }, i) => (
                        <PersonListItem key={person.id + i} person={person} cell={cell} tone="ok" />
                      ))}
                    </ul>
                  )}
  ...
  return (
    <li className="flex items-center justify-between gap-3 px-3 py-2.5">
      <div className="flex items-center gap-2 min-w-0 flex-1">
        <span
          className={`inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full ${groupColors} text-xs font-semibold`}
        >
          {initials}
        </span>
      ...
      <span
        className={`inline-flex items-center gap-1 rounded-full border px-2.5 py-1 text-[11px] font-semibold whitespace-nowrap ${statusBadgeColor}`}
      >
