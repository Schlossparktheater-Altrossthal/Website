  ...
  return (
    <section className="hidden sm:block">
      ...
      >
        ...
        <table
          className={`w-full ${compact ? "text-[12px]" : "text-sm"} table-fixed border-collapse [--th:theme(colors.border/.7)]`}
        >
          <thead className="sticky top-0 z-20 bg-card backdrop-blur shadow-sm">
              ...
              <>
                ...
                {hasFerien && (
                  <tr>
                    ...
                    <th
                      className={`sticky ${showGroupColumn ? "left-3" : "left-0"} z-20 w-[280px] sm:w-[340px] min-w-[280px] border-b border-r border-[color:var(--th)] bg-card px-3 py-2 text-right`}
                    >
                    ...
                    {dayCols.map((_, idx) => {
                      ...
                      return (
                        <th key={idx} className="border-b border-[color:var(--th)] bg-card"></th>
                      );
                    })}
                  </tr>
                )}
            ...
            <tr>
              {showGroupColumn && (
                <th
                  scope="col"
                  className="sticky left-0 z-20 w-3 min-w-[12px] border-b border-r border-[color:var(--th)] bg-card"
                ></th>
              )}
              <th
                scope="col"
            ...
            {groupedPeople ? (
              ...
              filteredPeople.map((p) => (
                <tr key={p.id} className="border-b border-border/50">
                  ...
                  {p.days.map((cell) => (
                    <td
                      key={`${p.id}-${cell.dayKey}`}
                      className="px-1.5 py-1 align-top whitespace-normal break-words"
                    >
...
function PersonInfo({ person, groupColor }: PersonInfoProps) {
  ...
  return (
    <div className="flex items-center gap-2">
      <span
        className={`flex h-8 w-8 items-center justify-center rounded-full ${avatarColor} text-xs font-semibold`}
      >
        {initials}
      </span>
