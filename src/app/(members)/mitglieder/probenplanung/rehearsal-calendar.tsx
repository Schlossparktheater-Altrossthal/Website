    ...
    <section className="space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          Die moderne Split-Ansicht kombiniert Monatskalender und
          Detailbereich. Wochenenden werden hervorgehoben, weil hier fast
          alle Proben stattfinden.
        </p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,7fr)_minmax(0,5fr)]">
            ...
            <div className="rounded-2xl border bg-background/80 p-4 shadow-sm">
              <div className="space-y-1">
                ...
                <p className="text-xs text-muted-foreground">
                  Freitag bis Sonntag immer im Blick. Tippe auf eine Karte,
                  um die Detailansicht unten zu öffnen.
                </p>
              </div>
                ...
                {weekendFocusDays.map((entry) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "group min-w-[180px] rounded-2xl border border-border/60 bg-card/60 p-4 text-left transition hover:border-primary/40 hover:shadow-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2",
                        isSelected &&
                    ...
                    >
                      ...
                      {timePreview ? (
                        <div className="mt-2 text-[10px] text-muted-foreground">
                          Frühester Start: <span className="font-semibold text-primary">{timePreview}</span>
                        </div>
                      ) : null}
                    </button>
                  );
        ...
        <aside className="rounded-2xl border bg-card/80 p-5 shadow-sm lg:sticky lg:top-24">
          <header className="space-y-1">
            ...
            {selectedSummary ? (
              ...
              <p className="text-sm text-muted-foreground">
                Wähle im Kalender einen Tag aus, um Details zu sehen.
              </p>
            )}
          </header>

          {selectedIsWeekend ? (
            <div className="mt-4 flex items-center gap-2 rounded-xl border border-primary/40 bg-primary/10 px-3 py-2 text-xs font-medium text-primary">
              <span className="h-2 w-2 rounded-full bg-primary" />
              Wochenend-Spot – ideale Zeit für Proben
            </div>
          ) : null}
              ...
              <ul className="space-y-2">
                {selectedDayRehearsals.map((entry) => {
                  ...
                  return (
                    <li
                      key={entry.id}
                      className="rounded-xl border border-border/60 bg-muted/40 px-3 py-2"
                    >
            ...
            {selectedDayBlocked.length ? (
              <ul className="space-y-2">
                {selectedDayBlocked.map((entry) => {
                  const displayName =
                    entry.user.name ?? entry.user.email ?? "Mitglied";
                  return (
                    <li
                      key={entry.id}
