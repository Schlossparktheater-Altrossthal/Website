    ...
    <section aria-labelledby="home-links-heading" className="mb-56 space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {LINK_CARDS.map(({ href, title, description, Icon }) => (
          <Card
            key={href}
            className="group flex h-full items-start border border-primary/60 bg-card/70 p-5 text-left shadow-sm transition duration-300 hover:-translate-y-0.5 hover:shadow-md"
          >
            <Link
              href={href}
              className="flex h-full w-full flex-col text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
            >
