...
export default async function ChroniclePage() {
      ...
      <div className="layout-container py-16">
        <div className="mx-auto max-w-2xl space-y-6 text-center">
          ...
          <Text tone="muted" align="center">
            Die Geschichten vergangener Sommer werden bald enthüllt. Hier werden die mystischen
            Momente unserer Aufführungen für die Ewigkeit bewahrt.
          </Text>
        </div>
      </div>
    );
  ...
  return (
    <div className="relative">
      ...
      <ChronikYearNavigation
        items={items.map((item) => ({ id: item.id, year: item.year, title: item.title }))}
      />
    </div>
  );
}
