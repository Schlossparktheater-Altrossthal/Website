...
function normalizePalette({ json, list }) {
  ...
  if (json) {
    ...
    if (Array.isArray(data)) {
      ...
      for (const [name, v] of Object.entries(data))
        entries.push({ name, fill: v.fill, stroke: v.stroke });
    } else {
  ...
  for (const e of entries) {
    if (!/^#[0-9a-fA-F]{6}$/.test(e.fill || ""))
      throw new Error(`Invalid fill for ${e.name}: '${e.fill}'`);
    if (e.stroke && !/^#[0-9a-fA-F]{6}$/.test(e.stroke))
      throw new Error(`Invalid stroke for ${e.name}: '${e.stroke}'`);
  }
  return entries;
}
...
function main() {
  try {
    ...
    for (const { name, fill, stroke } of palette) {
      ...
      adocLines.push(
        `image::${rel}[width=${Math.min(12, args.size)},height=${Math.min(12, args.size)}] ${name} (${fill}${stroke ? `/${stroke}` : ""})`,
      );
    }
    if (args.adoc) {
