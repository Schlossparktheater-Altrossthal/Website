...
function formatOklch({ l, c, h, alpha }) {
  const parts = [
    toPrecision(clamp(l, 0, 1), 3),
    toPrecision(Math.max(c, 0), 3),
    toPrecision(wrapHue(h), 1),
  ];
...
function resolveTokenValue(tokenName, mode) {
  ...
  const familyName =
    (modeConfig && typeof modeConfig === "object" && modeConfig.family) || definition.family;
...
const lightBlock = resolvedModes["light"]
  ? Object.entries(resolvedModes["light"])
      .map(([name, value]) => `  --${name}: ${value};`)
      .join("\n")
  : "";
const darkBlock = resolvedModes["dark"]
  ? Object.entries(resolvedModes["dark"])
      .map(([name, value]) => `  --${name}: ${value};`)
      .join("\n")
  : "";
